    ...
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(job_card)="row">
                    {{ row.item.job_card ? row.item.job_card.work_order.code : '-' }}
                </template>
                <template v-slot:cell(machine)="row">
                    {{ row.item.machine ? row.item.machine.code : '-' }}
                </template>
                <template v-slot:cell(failure_type)="row">
                    {{ row.item.failure_type ? row.item.failure_type.name : '-' }}
                </template>
        ...
        <b-modal id="form-modal" size="lg" :title="$t('form module',{module:$tc('backlog')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'job_card', label: 'job card', sortable: true },
                { key: 'machine', label: 'machine', sortable: true },
                { key: 'failure_type', label: 'failure type', sortable: true },
                { key: 'estimation_repair_time', label: 'estimation repair time', sortable: true },
                { key: 'estimation_endurance_time', label: 'estimation endurance time', sortable: true },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.backlog.tableParams
            },
            set(params) {
                this.$store.commit('backlog/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('backlog', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('backlog'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('backlog'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('backlog')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/engineering/backlog-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>