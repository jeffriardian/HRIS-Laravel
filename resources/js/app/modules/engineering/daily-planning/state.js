...
function defaultForm() {
    return {
        engineering_weekly_planning_id: null,
        day: 0,
        revision: 0,
        is_active: false,
        created_by: '', 
...
const mutations = {
    ...
    ASSIGN_FORM(state, payload) {
        state.form = {
            engineering_weekly_planning_id: payload.engineering_weekly_planning_id,
            day: payload.day,
...
const actions = {
    ...
    load({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/engineering/daily-plannings`,{ params: payload })
            .then((response) => {
    ...
    loadList({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/engineering/daily-planning-list`)
            .then((response) => {
                //SIMPAN DATA KE STATE MELALUI MUTATIONS
    ...
    store({ dispatch, commit, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //DARI STATE FORM
            $axios.post(`/engineering/daily-plannings`, state.form)
            .then((response) => {
    ...
    show({ commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.get(`/engineering/daily-plannings/${payload}`)
            .then((response) => {
    ...
    update({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //MELALUI STATE FORM
            $axios.put(`/engineering/daily-plannings/${payload}`, state.form)
            .then((response) => {
                //FORM DIBERSIHKAN
                commit('CLEAR_FORM')
                resolve(response.data)
    ...
    destroy({ dispatch, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.delete(`/engineering/daily-plannings/${payload}`)
            .then((response) => {
