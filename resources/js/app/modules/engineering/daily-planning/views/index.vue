    ...
    <div class="card">
        <div class="card-body">
            ...
            <b-table responsive="sm" thead-class="thead-light" small hover bordered :items="collection.data" :fields="fields" show-empty>
                ...
                <template v-slot:cell(week)="row">
                    {{ row.item.engineering_weekly_planning ? row.item.engineering_weekly_planning.week : '-' }}
                </template>
        ...
        <b-modal id="form-modal" size="xl" :title="$t('form module',{module:$tc('daily planning')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'week', label: 'week', thStyle: 'width: 40%;'},
                { key: 'day', label: 'day', thStyle: 'width: 40%;'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.dailyPlanning.tableParams
            },
            set(params) {
                this.$store.commit('dailyPlanning/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('dailyPlanning', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('daily planning'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('daily planning'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('daily planning')}), this.$t('warning'), {
