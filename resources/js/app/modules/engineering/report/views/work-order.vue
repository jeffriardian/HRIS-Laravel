        ...
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(is_active)="row">
                    <span class="badge badge-pill badge-soft-success font-size-12" v-if="row.item.is_active == 1">{{ $t('done') }}</span>
                    <span class="badge badge-pill badge-soft-danger font-size-12" v-else>{{ $t('on progress') }}</span>
                </template>
                <template v-slot:cell(created_at)="row">
                    {{ row.item.created_at | moment("from", "now") }}
                </template>
                <template v-slot:cell(physicalAvailability)="row">
                    {{ getPhysicalAvailability(row.item.job_cards)*100 }} %
                </template>
            </b-table>
        </div>
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'is_active', label: 'status', sortable: true, thStyle: 'text-align: center; width: 150px;', tdClass: 'text-center' },
    ...
    computed: {
        ...mapState('report', {
            collection: state => state.workOrder,
        }),
        tableParams: {
            get() {
    ...
    methods: {
        ...
        ...mapActions('report', {load:'workOrder'}),
        getWorkOrder(jobCards){
            var duration = 0;
        ...
        exportFile(type) {
            window.open(`/api/engineering/reports/work-order-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>