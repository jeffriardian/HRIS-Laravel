    ...
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(effectiveHour)="row">
                    {{ (row.item.name == 'Tantri Mulyani' || row.item.name == 'Mutia Halimah') ? '75%' : '0%' }} %
                </template>
            </b-table>
        </div>
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'effectiveHour', label: 'effective hour', thStyle: 'text-align: center; width: 180px;',tdClass: 'text-center'},
            ],
            keyword:'',
            modelId:null,
    ...
    computed: {
        ...mapState('report', {
            collection: state => state.effectiveHour,
        }),
        tableParams: {
            get() {
    ...
    methods: {
        ...
        ...mapActions('report', {load:'effectiveHour'}),
        getEffectiveHour(jobCards){
            // var duration = 0;
