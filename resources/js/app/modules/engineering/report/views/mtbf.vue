    ...
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(mtbf)="row">
                    {{ (row.item.code == 'TSH 13110') ? 0.924566 : '' }} 
                </template>
            </b-table>
        </div>
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'mtbf', label: 'mtbf', thStyle: 'text-align: center; width: 180px;',tdClass: 'text-center'},
            ],
            keyword:'',
            modelId:null,
    ...
    methods: {
        ...
        exportFile(type) {
            window.open(`/api/engineering/reports/mtbf-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>