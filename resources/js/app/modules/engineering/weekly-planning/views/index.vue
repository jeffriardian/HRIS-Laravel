<template>
    <div class="card">
        <div class="card-body">
            ...
            <!-- End Card Header -->
            <b-table responsive="sm" thead-class="thead-light" small hover bordered :items="collection.data" :fields="fields" show-empty>
                ...
                <template v-slot:cell(actions)="row">
                    <router-link :to="{ name: 'weeklyPlannings.view', params: {id: row.item.id} }" class="text-warning" v-b-tooltip.hover :title="$tc('view')"><i class="mdi mdi-table-eye font-size-16"></i></router-link>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('weekly planning')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'week', label: 'week', thStyle: 'width: 80%;',},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.weeklyPlanning.tableParams
            },
            set(params) {
                this.$store.commit('weeklyPlanning/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('weeklyPlanning', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                ...
                form.append('engineering_monthly_planning_id', this.form.engineering_monthly_planning_id);
                form.append('week', this.form.week);
                ...
                this.store(form).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('weekly planning'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('weekly planning'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('weekly planning')}), this.$t('warning'), {
