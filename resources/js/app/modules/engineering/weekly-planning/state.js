...
function defaultForm() {
    return {
        engineering_monthly_planning_id: null,
        week: 0,
        revision: 0,
        file: '',
        is_active: false,
...
const mutations = {
    ...
    ASSIGN_FORM(state, payload) {
        state.form = {
            engineering_monthly_planning_id: payload.engineering_monthly_planning_id,
            week: payload.week,
...
const actions = {
    ...
    load({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/engineering/weekly-plannings`,{ params: payload })
            .then((response) => {
    ...
    loadList({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/engineering/weekly-planning-list`)
            .then((response) => {
                //SIMPAN DATA KE STATE MELALUI MUTATIONS
    ...
    store({ dispatch, commit, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //DARI STATE FORM
            $axios.post(`/engineering/weekly-plannings`, payload,{
                headers: {
    ...
    show({ commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.get(`/engineering/weekly-plannings/${payload}`)
            .then((response) => {
    ...
    update({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //MELALUI STATE FORM
            $axios.put(`/engineering/weekly-plannings/${payload}`, state.form)
            .then((response) => {
                //FORM DIBERSIHKAN
                commit('CLEAR_FORM')
                resolve(response.data)
    ...
    destroy({ dispatch, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.delete(`/engineering/weekly-plannings/${payload}`)
            .then((response) => {
