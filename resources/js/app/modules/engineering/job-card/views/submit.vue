<template>
    <div>
        ...
        <div v-if="!this.workOrder || this.workOrder.work_order_action_id != 3" class="form-group" :class="{ 'has-error': errors.description }">
        ...
        <div class="card m-b-30">
            <div class="card-header bg-info">
                <div class="row align-items-center">
                    <div class="col-9">
                        <h5 class="card-title text-light">Part Used</h5>
                    </div>
                    <div class="col-3">
                        <button type="button" @click="addPartUsed()" class="btn btn-sm btn-rounded btn-secondary waves-effect waves-light float-right" v-b-tooltip.hover :title="$tc('add')"><i class="mdi mdi-plus"></i></button>
                    </div>
                </div>
            </div>
            <table class="table table-bordered table-sm table-hover">
                ...
                <tbody>
                    <tr v-for="(row, index) in form.part_used" :key="index" class="justify-content-between align-items-center">
                        <td>
                            <input type="text" class="form-control form-control-sm" :class="{ 'border-danger': errors[`partUsed.${index}.part`] }" v-model="row.part">
                            <p class="text-danger" v-if="errors[`partUsed.${index}.part`]">{{ errors[`partUsed.${index}.part`][0] }}</p>
                        </td>
                        <td>
                            <input type="number" class="form-control form-control-sm" :class="{ 'border-danger': errors[`partUsed.${index}.quantity`] }" v-model="row.quantity">
                            <p class="text-danger" v-if="errors[`partUsed.${index}.quantity`]">{{ errors[`partUsed.${index}.quantity`][0] }}</p>
                        </td>
                        <td class="text-center">
                            <a href='javascript:void(0)' @click="removePartUsed(index)" class="text-danger" v-b-tooltip.hover :title="$tc('delete')"><i class="mdi mdi-trash-can-outline font-size-16"></i></a>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="card m-b-30">
            <div class="card-header bg-warning">
                <div class="row align-items-center">
                    <div class="col-9">
                        <h5 class="card-title">Part Complains</h5>
                    </div>
                    <div class="col-3">
                        <button type="button" @click="addPartComplain()" class="btn btn-sm btn-rounded btn-success waves-effect waves-light float-right" v-b-tooltip.hover :title="$tc('add')"><i class="mdi mdi-plus"></i></button>
                    </div>
                </div>
            </div>
            <table class="table table-bordered table-sm table-hover">
                <thead>
                    <tr>
                        <th width="250px">{{ $tc('part') }}</th>
                        <th width="250px">{{ $tc('description') }}</th>
                        <th width="70px">{{ $t('action') }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(row, index) in form.part_complains" :key="index" class="justify-content-between align-items-center">
                        <td>
                            <input type="text" class="form-control form-control-sm" :class="{ 'border-danger': errors[`partComplain.${index}.part`] }" v-model="row.part">
                            <p class="text-danger" v-if="errors[`partComplain.${index}.part`]">{{ errors[`partComplain.${index}.part`][0] }}</p>
                        </td>
                        <td>
                            <textarea cols="5" rows="5" class="form-control form-control-sm" :class="{ 'border-danger': errors[`partComplain.${index}.quantity`] }" v-model="row.description"></textarea>
                            <p class="text-danger" v-if="errors[`partComplain.${index}.quantity`]">{{ errors[`partComplain.${index}.quantity`][0] }}</p>
                        </td>
                        <td class="text-center">
                            <a href='javascript:void(0)' @click="removePartComplain(index)" class="text-danger" v-b-tooltip.hover :title="$tc('delete')"><i class="mdi mdi-trash-can-outline font-size-16"></i></a>
                        </td>
                    </tr>
                </tbody>
            </table>
...
export default {
    ...
    methods: {
        ...
        //KETIKA TOMBOL TAMBAHKAN DITEKAN, MAKA AKAN MENAMBAHKAN ITEM BARU
        addPartUsed() {
            this.form.part_used.push({ part: null, quantity: null});
        },
        //KETIKA TOMBOL HAPUS PADA MASING-MASING ITEM DITEKAN, MAKA AKAN MENGHAPUS BERDASARKAN INDEX DATANYA
        removePartUsed(index) {
            if (this.form.part_used.length > 1) {
                this.form.part_used.splice(index, 1)
            }
        },
        //KETIKA TOMBOL TAMBAHKAN DITEKAN, MAKA AKAN MENAMBAHKAN ITEM BARU
        addPartComplain() {
            this.form.part_complains.push({ part: null, description: null});
        },
        //KETIKA TOMBOL HAPUS PADA MASING-MASING ITEM DITEKAN, MAKA AKAN MENGHAPUS BERDASARKAN INDEX DATANYA
        removePartComplain(index) {
            if (this.form.part_complains.length > 1) {
                this.form.part_complains.splice(index, 1)
            }
        },
    },
