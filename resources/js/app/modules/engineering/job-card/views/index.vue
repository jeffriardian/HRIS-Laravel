    ...
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(workOrder)="row">
                    {{ row.item.work_order ? row.item.work_order.code : '-' }}
                </template>
                <template v-slot:cell(machine)="row">
                    {{ row.item.machine ? row.item.machine.code : '-' }}
                </template>
                <template v-slot:cell(employee)="row">
                    {{ row.item.employee ? row.item.employee.name : '-' }}
                </template>
                ...
                <template v-slot:cell(actions)="row">
                    <a href='javascript:void(0)' @click="print(row.item.id)" class="text-primary" v-b-tooltip.hover :title="$tc('print')"><i class="mdi mdi-credit-card-multiple-outline font-size-16"></i></a>
                    <a href='javascript:void(0)' @click="view(row.item.id)" class="text-success" v-b-tooltip.hover :title="$tc('submit')"><i class="mdi mdi-file-table-box-outline font-size-16"></i></a>
        ...
        <b-modal id="form-modal" size="xl" :title="$t('form module',{module:$tc('job card')})" ref="modal">
        ...
        <b-modal id="submit-modal" size="xl" :title="$t('form module',{module:$tc('job card')})" ref="modal">
            <form v-on:submit.prevent="submit">
                <submitComponent :modelId=modelId></submitComponent>
            </form>
            <template v-slot:modal-footer>
                <b-button class="btn btn-secondary ml-2" @click="hideModal()"><i class="fas fa-arrow-left mr-2"></i> {{ $t('cancel') }}</b-button>
                <button class="btn btn-primary" @click.prevent="submit">
        ...
        <b-modal id="print-modal" size="xl" :title="$t('form module',{module:$tc('job card')})" ref="modal">
            <form v-on:submit.prevent="submit">
                <formComponent :modelId=modelId></formComponent>
            </form>
            <template v-slot:modal-footer>
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'workOrder', label: 'work order', sortable: true, thStyle: 'text-align: center;',tdClass: 'text-center'},
                { key: 'machine', label: 'machine', sortable: true, thStyle: 'text-align: center;',tdClass: 'text-center'},
                { key: 'employee', label: 'employee', sortable: true, thStyle: 'text-align: center;',tdClass: 'text-center'},
                { key: 'stop_at', label: 'stop at', sortable: true, thStyle: 'text-align: center;',tdClass: 'text-center'},
                { key: 'start_at', label: 'start at', sortable: true, thStyle: 'text-align: center;',tdClass: 'text-center'},
    ...
    components: {
        formComponent, submitComponent, printComponent
    },
    computed: {
        ...mapState('jobCard', {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.jobCard.tableParams
            },
            set(params) {
                this.$store.commit('jobCard/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('jobCard', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('job card'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('job card'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('job card')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/engineering/job-card-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>