<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table responsive="sm" thead-class="thead-light" small hover bordered :items="collection.data" :fields="fields" show-empty>
                ...
                <template v-slot:cell(year)="row">
                    {{ row.item.engineering_annual_planning ? row.item.engineering_annual_planning.year : '-' }}
                </template>
        ...
        <b-modal id="form-modal" size="xl" :title="$t('form module',{module:$tc('monthly planning')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'year', label: 'year', thStyle: 'width: 40%;'},
                { key: 'month', label: 'month', thStyle: 'width: 40%;'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.monthlyPlanning.tableParams
            },
            set(params) {
                this.$store.commit('monthlyPlanning/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('monthlyPlanning', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('monthly planning'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('monthly planning'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('monthly planning')}), this.$t('warning'), {
