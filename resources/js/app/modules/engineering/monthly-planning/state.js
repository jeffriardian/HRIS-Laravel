...
function defaultForm() {
    return {
        engineering_annual_planning_id: null,
        month: 0,
        is_active: false,
        created_by: '', 
        updated_by: '', 
...
const mutations = {
    ...
    ASSIGN_FORM(state, payload) {
        state.form = {
            engineering_annual_planning_id: payload.engineering_annual_planning_id,
            month: payload.month,
...
const actions = {
    ...
    load({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/engineering/monthly-plannings`,{ params: payload })
            .then((response) => {
    ...
    loadList({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/engineering/monthly-planning-list`)
            .then((response) => {
                //SIMPAN DATA KE STATE MELALUI MUTATIONS
    ...
    store({ dispatch, commit, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //DARI STATE FORM
            $axios.post(`/engineering/monthly-plannings`, state.form)
            .then((response) => {
    ...
    show({ commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.get(`/engineering/monthly-plannings/${payload}`)
            .then((response) => {
    ...
    update({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //MELALUI STATE FORM
            $axios.put(`/engineering/monthly-plannings/${payload}`, state.form)
            .then((response) => {
                //FORM DIBERSIHKAN
                commit('CLEAR_FORM')
                resolve(response.data)
    ...
    destroy({ dispatch, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            $axios.delete(`/engineering/monthly-plannings/${payload}`)
            .then((response) => {
