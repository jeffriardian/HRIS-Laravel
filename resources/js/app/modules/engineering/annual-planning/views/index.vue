<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table responsive="sm" thead-class="thead-light" small hover bordered :items="collection.data" :fields="fields" show-empty>
                ...
                <template v-slot:cell(actions)="row">
                    <router-link :to="{ name: 'annualPlannings.view', params: {id: row.item.id} }" class="text-warning" v-b-tooltip.hover :title="$tc('view')"><i class="mdi mdi-table-eye font-size-16"></i></router-link>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('annual planning')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'year', label: 'year', thStyle: 'width: 80%;',},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.annualPlanning.tableParams
            },
            set(params) {
                this.$store.commit('annualPlanning/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('annualPlanning', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                ...
                this.store(form).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('annual planning'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('annual planning'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('annual planning')}), this.$t('warning'), {
