<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(file)="row">
                    <a :href="`/storage/machines/`+row.item.file">{{ row.item.file }}</a>
                </template>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('machine library')})" ref="modal">
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.engineering.machineLibrary.tableParams
            },
            set(params) {
                this.$store.commit('machineLibrary/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('machineLibrary', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                ...
                form.append('machine_id', this.form.machine_id);
                ...
                this.store(form).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('machine library'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('machine library'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('machine library')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/engineering/service-type-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>