<template>
    <div class="card">
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-8">
                    <div class="float-right">
                        <button type="button" @click="updateData()" class="btn btn-sm btn-info-rgba">
                        <i class="feather icon-help-circle"></i>Update Data Salaries
                        </button>
                        <b-dropdown>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Manage Salary')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'employee', label: 'Employee Name', sortable: true, thStyle: 'text-align: center; width: 50px;',tdClass: 'text-center'},
                { key: 'component', label: 'Component Name', sortable: true, thStyle: 'text-align: center; width: 50px;',tdClass: 'text-center'},
                { key: 'amount', label: 'Amount', sortable: true, thStyle: 'text-align: center; width: 50px;',tdClass: 'text-center'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.salary.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/salary/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('salary', ['load','show', 'store', 'update', 'destroy','updateDataSalaries']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Salary'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Salary'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Salary')}), this.$t('warning'), {
        ...
        updateData() {
        this.$confirm(
            this.$t("Update Data Salary Confirmation?", {
            module: this.$tc("Update Data Salaries"),
            }),
            this.$t("Confirmation"),
            {
            confirmButtonText: this.$t("ok"),
            ...
            this.$message({
                type: "info",
                message: this.$t("update data canceled"),
            });
            });
        },
    }
