<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('PPH 21')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'company_allowance', label: 'Allowance', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'bruto', label: 'Bruto', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'deduction', label: 'Deduction', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'netto_month', label: 'Netto (Month)', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'netto_year', label: 'Netto (Year)', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'ptkp', label: 'PTKP', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'pkp', label: 'PKP', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'pph21_owed_year', label: 'PPH 21 Terutang (Year)', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'pph21_owed_month', label: 'PPH 21 Terutang (Month)', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'month', label: 'Month', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'year', label: 'Year', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.pph.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/pph/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('pph', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('PPH 21'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('PPH 21'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('PPH 21')}), this.$t('warning'), {
