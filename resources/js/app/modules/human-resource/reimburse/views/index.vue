<template>
    <div class="card">
        ...
        <b-modal id="form-modal" size="lg" :title="$t('form module',{module:$tc('Request for Reimburse')})" ref="modal">
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.reimburse.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/reimburse/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('reimburse', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                ...
                form.append('reimburse_category_id', this.form.reimburse_category_id);
                form.append('employee_id', this.form.employee_id);
                form.append('reimburse_number', this.form.reimburse_number);
                form.append('description', this.form.description);
                form.append('total_cost', this.form.total_cost);
                form.append('reimburse_type', this.form.reimburse_type);
                form.append('approval_status_id', this.form.approval_status_id);
                ...
                this.store(form).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Reimburse'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Reimburse'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Reimburse')}), this.$t('warning'), {
