<template>
    <div class="card">
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-8">
                    <div class="float-right">
                        <b-button @click="getDataAPIHoliday()" class="btn btn-success ml-2"><i class="mdi mdi-cloud-download"></i> {{ $tc('Get Holiday') }}</b-button>
                        <b-dropdown>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Public Holiday')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'holiday_date', label: 'Date of Holiday', sortable: true, thStyle: 'text-align: center; width: 35%;', tdClass: 'text-center' },
                { key: 'description', label: 'Description', sortable: true, thStyle: 'text-align: center; width: 35%;', tdClass: 'text-center' },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.publicHoliday.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/publicHoliday/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('publicHoliday', ['load','show', 'store', 'update', 'destroy', 'apiHoliday']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Public Holiday'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Public Holiday'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        ...
        {
            this.loadingProcess = true;
            this.apiHoliday().then(() => {
                this.$message({
                    type: 'success',
                    message: this.$t('get data from API successfully')
                });
            });
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Public Holiday')}), this.$t('warning'), {
