<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(actions)="row">
                    <a href='javascript:void(0)' :href="`/storage/images/employee-photos/${row.item.file}`" target="blank" class="text-primary" v-b-tooltip.hover :title="$tc('Download Attachment')"><i class="mdi mdi-download font-size-16"></i></a>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('employee status')})" ref="modal">
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.historyEmployeePosition.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/historyEmployeePosition/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('historyEmployeePosition', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                ...
                this.store(form).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('employee status'),message:this.$tc('add',2)})
                    });
                });
            }else{
                let form = new FormData();
                ...
                this.update({"id":this.modelId,"form":form}).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('employee status'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.contract_id, module:this.$tc('employee status')}), this.$t('warning'), {
