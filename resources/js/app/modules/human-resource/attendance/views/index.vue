<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Attendance')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'payrolltype', label: 'Payroll Type', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'date_work', label: 'Date', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'time_in', label: 'Schedule IN', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'in_time', label: 'Time IN', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'time_out', label: 'Schedule OUT', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'out_time', label: 'Time OUT', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'status', label: 'Status', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
                { key: 'total_time_work', label: 'Total Time Work', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.attendance.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/attendance/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('attendance', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Attendance'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Attendance'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Attendance')}), this.$t('warning'), {
