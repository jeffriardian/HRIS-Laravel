<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Cooperative Member')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'id_member', label: 'Member Number', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'name', label: 'Employee', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'company', label: 'Company', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'department', label: 'Department', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'member_type', label: 'Member Type', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'join_date', label: 'Join Date', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'saving_balance', label: 'Saving Balance', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'loan_balance', label: 'Loan Balance', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'installment_count', label: 'Installment Count', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'installment_payment', label: 'Installment Payment', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.cooperativeMember.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/cooperativeMember/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('cooperativeMember', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Cooperative Member'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Cooperative Member'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Cooperative Member')}), this.$t('warning'), {
