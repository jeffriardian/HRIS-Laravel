...
export default {
    ...
    methods: {
        ...
        ...mapMutations('loanTransaction', ['CLEAR_FORM']), //PANGGIL MUTATIONS CLEAR_FORM
        ...mapActions('cooperativeMember', {loadCooperativeMembers:'loadList'}),

        changeTotal(){
            var amount = parseInt(this.form.amount);
            var interest = ((parseInt(this.form.interest))/100)*parseInt(this.form.amount);
            ...
            if (this.form.installment_count != '0'){
                this.form.payment = total/count;
            }else{
                this.form.payment = total;
            }
        }
    },
