<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(actions)="row">
                    <a href='javascript:void(0)' @click="view(row.item.id)" class="text-primary" v-b-tooltip.hover :title="$tc('Return')"><i class="mdi mdi-exit-to-app font-size-16"></i></a>
                </template>
            </b-table>
        </div>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Return Inventory')})" ref="modal">
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.inventoryReturn.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/inventoryReturn/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('inventoryReturn', ['load','show', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Return Inventory'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Return Inventory'),message:this.$tc('update',2)})
                    });
                })
            }
        },
