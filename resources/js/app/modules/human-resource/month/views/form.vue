<template>
    <div>
        <div class="form-group" :class="{ 'has-error': errors.id }">
            <label for="">{{ $t('Month Code') }}</label>
            <input type="text" class="form-control" :class="{ 'border-danger': errors.id }" v-model="form.id">
            <p class="text-danger" v-if="errors.id">{{ errors.id[0] }}</p>
        </div>
        <div class="form-group" :class="{ 'has-error': errors.name }">
            <label for="">{{ $t('Month Name') }}</label>
...
export default {
    ...
    methods: {
        ...
        ...mapMutations('month', ['CLEAR_FORM']), //PANGGIL MUTATIONS CLEAR_FORM
    },
