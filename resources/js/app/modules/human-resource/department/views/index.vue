<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(level)="row">
                    {{ row.item.department_level.name }}
                </template>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('department')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'code', label: 'code', thStyle: 'text-align: center; width: 50px;',tdClass: 'text-center'},
                { key: 'level', label: 'level', thStyle: 'width: 180px;'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.department.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/department/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('department', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('department'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('department'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('department')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/human-resource/department-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>