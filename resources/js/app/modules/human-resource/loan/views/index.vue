<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Loan')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'loan_date', label: 'Loan Date', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'amount', label: 'Amount', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'interest', label: 'Interest', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                { key: 'total', label: 'Total Loan', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                // { key: 'balance', label: 'Loan Balance', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                // { key: 'installment_count', label: 'Installment Count', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
                // { key: 'payment', label: 'Payment', sortable: true, thStyle: 'text-align: center;', tdClass: 'text-center' },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.loan.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/loan/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('loan', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Loan'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Loan'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Loan')}), this.$t('warning'), {
