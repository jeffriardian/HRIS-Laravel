<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Payroll Component Type')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'name', label: 'name', sortable: true, thStyle: 'text-align: center; width: 30%;' },
                { key: 'type', label: 'Type', sortable: true, thStyle: 'text-align: center; width: 10%;' ,tdClass: 'text-center' },
                { key: 'time', label: 'Time', sortable: true, thStyle: 'text-align: center; width: 10%;',tdClass: 'text-center' },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.payrollComponentType.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/payrollComponentType/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('payrollComponentType', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Payroll Component Tyoe'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Payroll Component Type'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Payroll Component Type')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/human-resource/payrollcomponent-type-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>
