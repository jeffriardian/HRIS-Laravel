<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Payroll Parameter')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'name', label: 'name', sortable: true, thStyle: 'text-align: center; width: 40%;',tdClass: 'text-center' },
                { key: 'type', label: 'type', sortable: true, thStyle: 'text-align: center; width: 40%;',tdClass: 'text-center' },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.payrollParameter.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/payrollParameter/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('payrollParameter', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Payroll Parameter'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Payroll Parameter'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Payroll Parameter')}), this.$t('warning'), {
