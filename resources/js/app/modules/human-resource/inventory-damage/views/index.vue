<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(actions)="row">
                    <a href='javascript:void(0)' @click="view(row.item.id)" class="text-primary" v-b-tooltip.hover :title="$tc('Return')"><i class="mdi mdi-pencil-box-outline font-size-16"></i></a>
                </template>
            </b-table>
        </div>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Request Inventory')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'employee', label: 'Employee', thStyle: 'text-align: center; width: 150px;', sortable: true, tdClass: 'text-center'},
                { key: 'inventory', label: 'Inventory', thStyle: 'text-align: center; width: 80px;', sortable: true, tdClass: 'text-center'},
                { key: 'type', label: 'Type', thStyle: 'text-align: center; width: 80px;', sortable: true, tdClass: 'text-center'},
                { key: 'serial', label: 'Serial Number', thStyle: 'text-align: center; width: 150px;', sortable: true, tdClass: 'text-center'},
                { key: 'date_borrow', label: 'Date Borrow', thStyle: 'text-align: center; width: 75px;', sortable: true, tdClass: 'text-center'},
                { key: 'date_return', label: 'Date Return', thStyle: 'text-align: center; width: 75px;', sortable: true, tdClass: 'text-center'},
                { key: 'status', label: 'Inventory Status', thStyle: 'text-align: center; width: 75px;', sortable: true, tdClass: 'text-center'},
                { key: 'compensation', label: 'Compensation', thStyle: 'text-align: center; width: 75px;', sortable: true, tdClass: 'text-center'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.inventoryDamage.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/inventoryDamage/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('inventoryDamage', ['load','show', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Request Inventory'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Request Inventory'),message:this.$tc('update',2)})
                    });
                })
            }
        },
