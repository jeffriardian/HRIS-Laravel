    ...
    <div class="card">
        <div class="card-body">
            ...
            >
                ...
                <template
                    v-slot:cell(interrogation_report_actor.interrogation_report_id)="
                        row
                    "
                >
                    ...
                    <a
                        ...
                        @click="
                            viewInterrogation(
                                row.item.interrogation_report_actor
                        ...
                        >{{
                            row.item.interrogation_report_actor
                                .interrogation_report.code
                        }}</a
                    >
                </template>
                <template
                    v-slot:cell(interrogation_report_actor.employee.name)="row"
                >
                    <!-- `data.value` is the value after formatted by the Formatter -->
                    <div
                        v-if="
                            row.item.interrogation_report_actor.employee_id != 0
                        "
                    >
        ...
        <b-modal
            id="form-modal"
            :title="$t('form module', { module: $tc('memorandum') })"
            ref="modal"
        >
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                {
                    key: "interrogation_report_actor.interrogation_report_id",
                    label: "Interrogation Report",
                    sortable: true,
                    tdClass: "text-center"
                },
                {
                    key: "interrogation_report_actor.employee.name",
                    label: "Employee",
                    sortable: true
                },
                {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.memorandum.tableParams;
            },
            set(params) {
                this.$store.commit(
                    "humanResource/memorandum/SET_TABLE_PARAMS",
                    params
                );
            }
        }
    ...
    methods: {
        ...
        exportFile(type) {
            window.open(
                `/api/human-resource/memorandum-export-${type}?api_token=${this.$store.state.token}`
            );
        }
    }
};
