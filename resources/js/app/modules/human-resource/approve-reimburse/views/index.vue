<template>
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(actions)="row">
                    ...
                    <a href='javascript:void(0)' @click="appReimburse(row.item)" class="text-danger" v-b-tooltip.hover :title="$tc('Approve')"><i class="mdi mdi-checkbox-marked font-size-16"></i></a>
                    <a href='javascript:void(0)' @click="remove(row.item)" class="text-danger" v-b-tooltip.hover :title="$tc('Reject')"><i class="mdi mdi-close-circle font-size-16"></i></a>
                </template>
            </b-table>
        </div>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Approve Reimburse')})" ref="modal">
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.approveReimburse.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/approveReimburse/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('approveReimburse', ['load','show', 'store', 'update', 'destroy', 'approve']),
        hideModal(){
        ...
        remove(item) {
            this.$confirm(this.$t('Approve Confirmation?',{item:item.employee, module:this.$tc('Approve Reimburse')}), this.$t('Confirmation'), {
