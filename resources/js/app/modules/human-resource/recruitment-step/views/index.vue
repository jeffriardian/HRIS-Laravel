<template>
    <div class="card">
        ...
        <b-modal
            id="form-modal"
            size="lg"
            :title="$t('form module', { module: $tc('recruitment step') })"
            ref="modal"
        >
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: "min_score", label: "Minimal Score", sortable: true },
                {
                    key: "is_active",
                    label: "status",
                    sortable: true,
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.recruitmentStep
                    .tableParams;
            },
            set(params) {
                this.$store.commit(
                    "humanResource/recruitmentStep/SET_TABLE_PARAMS",
                    params
                );
            }
        }
    ...
    methods: {
        ...
        hideModalEdit() {
            ...
            this.saveDisabled = false;
            this.load(this.tableParams);
            this.$bvModal.hide("form-edit");
        },
        view(id) {
            this.show(id).then(() => {
                this.$bvModal.show("form-edit");
            });
            this.modelId = id;
        },
        submit() {
                ...
                this.store().then(() => {
                    ...
                    this.$message({
                        ...
                        message: this.$t("present perfect", {
                            object: this.$tc("recruitment step"),
        ...
        remove(item) {
            this.$confirm(
                this.$t("delete confirmation", {
                    item: item.name,
                    module: this.$tc("recruitment step")
                }),
                this.$t("warning"),
                {
        ...
        exportFile(type) {
            window.open(
                `/api/human-resource/recruitment-step-export-${type}?api_token=${this.$store.state.token}`
            );
        }
    }
};
