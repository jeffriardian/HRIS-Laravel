<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('exit Clearance Parameter')})" ref="modal">
...
export default {
    ...
    computed: {
        ...mapState('exitClearanceParameter', {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.exitClearanceParameter.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/exitClearanceParameter/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('exitClearanceParameter', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Exit Clearance Parameter'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Exit Clearance Parameter'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Exit Clearance Parameter')}), this.$t('warning'), {
