<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Approve Overtime')})" ref="modal">
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.approveOvertime.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/approveOvertime/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('approveOvertime', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        remove(item) {
            this.$confirm(this.$t('Approve Confirmation?',{item:item.employee, module:this.$tc('Approve Overtime')}), this.$t('Confirmation'), {
