<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('schedule recruitment')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'recruitment_date', label: 'Recruitment Date', sortable: true },
                { key: 'status', label: 'Status Recruitment', sortable: true },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.scheduleRecruitment.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/scheduleRecruitment/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('scheduleRecruitment', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('schedule recruitment'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('schedule recruitment'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('schedule recruitment')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/human-resource/schedule-recruitment-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>