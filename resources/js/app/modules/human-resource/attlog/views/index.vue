<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('Finger Print Attendance')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'pin', label: 'PIN', sortable: true, thStyle: 'text-align: center; width: 120px;', tdClass: 'text-center' },
                { key: 'employee', label: 'Employee', sortable: true, thStyle: 'text-align: center; width: 120px;', tdClass: 'text-center' },
                { key: 'scan_date', label: 'Scan Date', sortable: true, thStyle: 'text-align: center; width: 120px;', tdClass: 'text-center' },
                { key: 'sn', label: 'Serial Number', sortable: true, thStyle: 'text-align: center; width: 50px;',tdClass: 'text-center'},
                { key: 'inoutmode', label: 'In Out Mode', sortable: true, thStyle: 'text-align: center; width: 120px;', tdClass: 'text-center' },
            ],
            keyword:'',
            modelId:null,
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.humanResource.attlog.tableParams
            },
            set(params) {
                this.$store.commit('humanResource/attlog/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('attlog', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Finger Print Attendance'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('Finger Print Attendance'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('Finger Print Attendance')}), this.$t('warning'), {
