    ...
    <div class="card">
        <div class="card-body">
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                responsive="sm" small striped hover bordered :items="collection.data" :fields="fields" show-empty>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('permission')})" ref="modal">
            <form v-on:submit.prevent="submit">
                <form-permission :modelId=modelId></form-permission>
            </form>
...
import FormPermission from './form.vue'
export default {
    created() {
    ...
    data() {
        return {
            fields: [
                ...
                //{ key: 'guard_name', label: 'guard_name', thStyle: 'text-align: center; width: 150px;', tdClass: 'text-center'},
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.general.permission.tableParams
            },
            set(params) {
                this.$store.commit('permission/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('permission', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('permission'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('permission'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('permission')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/permission-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>