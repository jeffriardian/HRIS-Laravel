<template>
    <div class="card">
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('role')})" ref="modal">
            <form v-on:submit.prevent="submit">
                <form-role :modelId=modelId></form-role>
            </form>
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.general.role.tableParams
            },
            set(params) {
                this.$store.commit('role/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('role', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                ...
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('role'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('role')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/role-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>