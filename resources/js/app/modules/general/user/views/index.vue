<template>
    <div class="card">
        <div class="card-body">
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-8">
                    <div class="float-right">
                        ...
                        <router-link :to="{ name: 'users.assignment' }" class="btn btn-info ml-2"><i class="bx bx-lock mr-1"></i> {{ $tc('assignment') }}</router-link>
            ...
            <b-table :sort-by.sync="tableParams.order_column" :sort-desc.sync="tableParams.order_direction"
                ...
                <template v-slot:cell(role)="row">
                    <span v-for="(role, index) in row.item.roles" :key="index"
                        class="badge badge-light badge-striped badge-striped-left border-left-primary">
                    {{ role.name }}
                    </span>                    
                </template>
        ...
        <b-modal id="form-modal" :title="$t('form module',{module:$tc('user')})" ref="modal">
            <form v-on:submit.prevent="submit">
                <form-user :modelId=modelId></form-user>
            </form>
...
export default {
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.general.user.tableParams
            },
            set(params) {
                this.$store.commit('user/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('user', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('user'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('user'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('user')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/user-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>