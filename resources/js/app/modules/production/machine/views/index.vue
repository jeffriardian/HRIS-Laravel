<template>
    <div class="card">
        ...
        <b-modal id="form-modal" size="lg" scrollable  :title="$t('form module',{module:$tc('machine')})" ref="modal">
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'year', label: 'year', sortable: true, thStyle: 'text-align: center; width: 150px;',tdClass: 'text-center' },
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.production.machine.tableParams
            },
            set(params) {
                this.$store.commit('production/machine/SET_TABLE_PARAMS', params)
            }
        },
    },
    watch: {
    ...
    methods: {
        ...
        ...mapActions('machine', ['load','show', 'store', 'update', 'destroy']),
        hideModal(){
        ...
        submit() {
            ...
            if(!this.modelId){
                this.store().then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('machine'),message:this.$tc('add',2)})
                    });
                });
            }else{
                this.update(this.modelId).then(() => {
                    ...
                    this.$message({
                        type: 'success',
                        message: this.$t('present perfect',{object:this.$tc('machine'),message:this.$tc('update',2)})
                    });
                })
            }
        },
        remove(item) {
            this.$confirm(this.$t('delete confirmation',{item:item.name, module:this.$tc('machine')}), this.$t('warning'), {
        ...
        exportFile(type) {
            window.open(`/api/production/machine-export-${type}?api_token=${this.$store.state.token}`)
        },
    }
}
</script>